...
- **Educational Book & Series Generator**: Dedicated generator for educational content (fiction and non-fiction) for grades K–12. Features: age group selection (Elementary K-2, 3-5, Middle School, High School), educational subjects (STEM, Historical Fiction, Biography, History & Social Studies, Geography, Civics, etc.), automatic fact-checking and bibliography for applicable subjects, and complete multi-book series generation (1-8 books) in a single generation session. All books in a series share a coherent "Series Bible" generated by GPT-5.2. Accessed via the "Educational" tab in the Create Hub. DB fields: `age_group`, `educational_subject`, `series_id`, `series_title`, `series_position`, `total_books_in_series`, `series_bible`.
- **Video Trailer Script Generator**: Per-book action in the library that generates a cinematic book trailer script (30s–10 min) using GPT-5.2. Scene-by-scene breakdown with visuals, voiceover, on-screen text, sound direction, call to action, and production notes. Copy to clipboard supported.
